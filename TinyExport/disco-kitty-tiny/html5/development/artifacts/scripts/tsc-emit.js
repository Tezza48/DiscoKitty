...
(function (game) {
    /** New System */
    var moveToPointerSystem = /** @class */ (function (_super) {
        __extends(moveToPointerSystem, _super);
        function moveToPointerSystem() {
            return _super !== null && _super.apply(this, arguments) || this;
        }
        moveToPointerSystem.prototype.OnUpdate = function () {
            var _this = this;
            var dt = this.scheduler.deltaTime();
            var pointerPosition = ut.Core2D.Input.getInputPosition();
            var worldPointer = ut.Core2D.Input.getWorldInputPosition(this.world);
            this.world.forEach([ut.Entity, game.moveToPointerComponent], function (entity, moveToPointer) {
                _this.world.usingComponentData(entity, [ut.Core2D.TransformLocalPosition], function (position) {
                    var direction = worldPointer.sub(position.position);
                    // dont normalize so that it's faster at a distance and slower when close
                    var delta = direction.multiplyScalar(dt * moveToPointer.moveSpeed);
                    position.position.add(delta);
                });
            });
        };
        return moveToPointerSystem;
    }(ut.ComponentSystem));
    game.moveToPointerSystem = moveToPointerSystem;
})(game || (game = {}));
var ut;
(function (ut) {
